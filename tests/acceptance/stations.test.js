...
async function getAdminToken() {
  const user = await registerUser({ name: 'Admin', email: 'admin@sta.com', password: 'Admin@1234', role: 'HQ_ADMIN' });
...
describe('GET /api/v1/stations', () => {
  it('should return 200 with paginated data', async () => {
    ...
    const res = await request(app).get('/api/v1/stations').set('Authorization', `Bearer ${token}`);
...
describe('POST /api/v1/stations', () => {
  it('should create a station and return 201 with Location header', async () => {
    ...
    expect(res.headers.location).toContain('/api/v1/stations/');
  });
  ...
  it('should return 403 if STATION role tries to create', async () => {
    const stationUser = await registerUser({ name: 'S', email: 'station@sta.com', password: 'Test@1234', role: 'STATION' });
...
describe('PUT /api/v1/stations/:id', () => {
  it('should allow partial update', async () => {
    ...
    const updateRes = await request(app)
      .put(`/api/v1/stations/${createRes.body._id}`)
      .set('Authorization', `Bearer ${token}`)
...
describe('DELETE /api/v1/stations/:id', () => {
  it('should return 204 on successful delete', async () => {
    ...
    const deleteRes = await request(app)
      .delete(`/api/v1/stations/${createRes.body._id}`)
      .set('Authorization', `Bearer ${token}`);
